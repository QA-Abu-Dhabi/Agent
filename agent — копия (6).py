...
class PDFProcessor(QWidget):
    ...
    def process_files(self):
        ...
        pdf_files_kc2 = self.find_pdf_files('Печатная форма*КС2*.pdf')
        pdf_files_oa = self.find_pdf_files('*ОА*.pdf')
        pdf_files_sf = self.find_pdf_files('Печатная форма Счет-фактура*.pdf')
    ...
    def find_pdf_files(self, pattern):
        # Поиск файлов по шаблону
        file_pattern = os.path.join(self.folder_path, '**', pattern)
